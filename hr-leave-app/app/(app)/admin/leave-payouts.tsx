...
export default function LeavePayoutsScreen() {
  ...
  const columns = useMemo(() => ([
    { field: 'emp_code', headerName: 'Emp #', width: 90, valueGetter: (p: any) => p.row.emp_code || '—' },
    { field: 'full_name', headerName: 'Employee', flex: 1.5, minWidth: 180 },
    { field: 'department', headerName: 'Dept', flex: 1, minWidth: 130, valueGetter: (p: any) => p.row.department || '—' },
    { field: 'basic_salary', headerName: 'Basic/mo', flex: 0.9, minWidth: 110, type: 'number', valueFormatter: (p: any) => formatMoney(Number(p.value) || 0) },
    { field: 'hra', headerName: 'HRA/mo', flex: 0.9, minWidth: 110, type: 'number', valueFormatter: (p: any) => formatMoney(Number(p.value) || 0) },
    { field: 'transportation', headerName: 'Transport/mo', flex: 0.9, minWidth: 110, type: 'number', valueFormatter: (p: any) => formatMoney(Number(p.value) || 0) },
    { field: 'days_in_month', headerName: 'Days off', flex: 0.7, minWidth: 90, type: 'number',
      valueFormatter: (p: any) => Number(p.value || 0).toFixed(p.value && p.value % 1 !== 0 ? 2 : 0) },
    { field: 'basic_payable', headerName: 'Basic Pay', flex: 1, minWidth: 110, type: 'number',
      valueFormatter: (p: any) => formatMoney(Number(p.value) || 0),
